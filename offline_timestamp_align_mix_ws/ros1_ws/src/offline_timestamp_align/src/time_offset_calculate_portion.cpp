...
std::vector<double> interpolate(const std::vector<VisData>& vis,
    ...
    for (double t : imu_t) {
        ...
        if (j + 1 >= vis.size()) {
            vis_interp.push_back(vis.back().wx); //越界用最后一个填充
            continue;
        }
        //线性插值，根据时间差计算比例
...
int main() {
    //定义路径
    std::string imu_csv = "/home/slam/20251212_ros1/imu0_data.csv";
    std::string vis_csv = "/home/slam/20251212_ros1/visual_angular_velocity_mid_cpp.csv";
    ...
    for (std::string axis : {"x", "y", "z"}) {
        ...
        //搜索当前轴的最佳偏移，范围是[-25,+25]ms,步长是1ms
